...
| 絞り込み | アーティスト・開催状況（参戦予定 / 参戦済み / すべて）・種別・年・月（都道府県は制覇マップ側で絞る） |
| 並び替え | 新しい順 / 古い順 / アーティスト順 |
| ライブの詳細 | カードをタップすると専用ページ（`/lives/[id]`）へ。編集・削除もここで行う |
| アーティスト | 応援開始日を設定すると「応援して 11年144日」のように経過を表示。初参戦日・参戦回数・次の参戦もまとめて確認できる（リンクとメモも保存可） |
| 新規登録 | モーダルで登録。同じアーティスト × 同じ日付は重複警告を出す |
...
### artists テーブル（アーティストごとの設定）

| 列 | 型 | 説明 |
| --- | --- | --- |
| id | uuid（主キー） | |
| user_id | uuid（→ auth.users.id） | |
| name | text | アーティスト名（ユーザー内で一意） |
| fan_since | date | 応援開始日。「応援して〇日目」の起点 |
| memo / url | text | メモ・公式サイトなどのリンク |
